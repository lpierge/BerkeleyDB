...
public:
    ...
    //
    int db_register(Db *dbp, const char *name, BERKLEYDB_DB_TYPE type, u_int32_t *fidp);
...
public:
    ...
    BERKLEYDB_DB_TYPE get_type() const;

    static int open(const char *fname, BERKLEYDB_DB_TYPE type, u_int32_t flags,
                    int mode, DbEnv *dbenv, DbInfo *info, Db **dbpp);

    static int xa_open(const char *fname, BERKLEYDB_DB_TYPE type, u_int32_t flags,
