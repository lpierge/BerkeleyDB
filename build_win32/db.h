 ...
 */
/* MSVC2008: da DBTYPE a BERKLEYDB_DB_TYPE perche' gia' definito in OLE
   MSVC2008: da DB_UNKNOWN a DB_NOTKNOWN perche' gia' definito in ODBC
   MSVC2008: con la typedef enum da errore
*/
#if _MSC_VER >= 1500 /* MSVC2008 */
	typedef int BERKLEYDB_DB_TYPE;
	#define DB_BTREE 1
	#define DB_HASH 2
	#define DB_RECNO 3
	#define DB_NOTKNOWN 4
#else
	typedef enum
	{
		DB_BTREE=1,			/* B+tree. */
		...
		DB_NOTKNOWN			/* Figure it out on open. */
	} BERKLEYDB_DB_TYPE;
#endif
...
struct __db {
	...
	BERKLEYDB_DB_TYPE	 type;			/* DB access method. */
...
int   db_open __P((const char *,
	  BERKLEYDB_DB_TYPE, u_int32_t, int, DB_ENV *, DB_INFO *, DB **));
int   db_value_set __P((int, int));
char *db_version __P((int *, int *, int *));
int   db_xa_open __P((const char *,
	  BERKLEYDB_DB_TYPE, u_int32_t, int, DB_INFO *, DB **));
#if defined(__cplusplus)
}
#endif
...
int	 log_register __P((DB_LOG *, DB *, const char *, BERKLEYDB_DB_TYPE, u_int32_t *));
